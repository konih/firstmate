...
| `fm-ci-watch.sh`         | Scheduled poll that delivers each new failed default-branch CI run to the owning repository's INBOX and a firstmate note ([configuration.md](configuration.md#ci-failure-watch-configci-watch-scope-configci-watch-inbox-root-configci-watch-inbox-map)) |
| `fm-ci-rescan.sh`        | One-shot Markdown survey of CI failure rate, time-to-green, red workflows, and flaky candidates |
| `fm-ci-lib.sh`           | Shared scope resolution for the CI watch and rescan                                  |
