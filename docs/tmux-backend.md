...
OpenCode 1.18.31 also draws its working directory and branch right-aligned inside the left-bar rows above the Build footer, and the classifier recognises that cwd:branch furniture structurally so an idle pane keeps a proven `empty` verdict.
